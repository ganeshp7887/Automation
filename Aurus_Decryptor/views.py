...
from Home.models import DecryptorKeys
from .models import decrypt, decryptData
import base64

class Aurus_Decryptor:

    def __init__(self) :
        self.PrivateKey = DecryptorKeys.objects.get(KeyName="PrivateKey")
        self.DefaultKey = DecryptorKeys.objects.get(KeyName="DefaultKey")
    ...
    def Decryptor(self, request):
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                context = {"EncryptedData" : Payload, "DecryptedData" : f'Enter valid json :: {e.msg} Error at line {e.lineno}, column {e.colno}, Error position: {e.pos}'}
            ...
            try :
                ...
                if encryptionFlag in ("05", "02") :
                    static_key = f"{self.DefaultKey}{txnDateTime}"
                    decryptText = decryptData(static_key, DataToDecrypt)
                    print(f"this is decrypted text :: {decryptText}")
                    decrypted_data = base64.b64decode(decryptText) if encryptionFlag == "05" else decryptText
                    decrypted_data = str(decrypted_data.decode('utf-8'))
                if encryptionFlag in ("01", "03", "06", "99"):
                    deviceSerialNumber = request.POST.get("deviceSerialNumber", "")
                    static_key = f"{self.PrivateKey}{deviceSerialNumber}{txnDateTime}"
